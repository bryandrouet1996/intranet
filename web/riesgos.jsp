...
<%@page import="modelo.FichaRiesgos"%>
<%@page import="modelo.ConexionRiesgos"%>
...
<%
    ...
    ConexionRiesgos mysqlR = new ConexionRiesgos();
    int id = 0;
    ...
    int funcionario = -1;
    ArrayList<FichaRiesgos> fichas = new ArrayList<>();
    try {
        id = Integer.parseInt(sesion.getAttribute("user").toString());
        if (!(enlace.verificarUsuarioCumpleRol(id, "admin_riesgos") || enlace.verificarUsuarioCumpleRol(id, "riesgos"))) {
        ...
        listadoFuncionarios = enlace.listadoUsuarioUnidad("D.02.15");
        ...
        if(request.getParameter("tecnico")!= null){
            funcionario = Integer.parseInt(request.getParameter("tecnico").toString());
        }
        fichas = request.getParameter("txtini") == null ? (funcionario == -1 ? mysqlR.getFichas() : mysqlR.getFichas()) : (funcionario == -1 ? mysqlR.getFichasFecha(fecha_inicio, fecha_fin) : mysqlR.getFichasUsuarioFecha(funcionario, fecha_inicio, fecha_fin));
...
<html lang="es">
    <head>
        ...
        <title>Intranet Alcaldía - Riesgos</title>
        <!-- General CSS Files -->
                                            ...
                                            <form action="">
                                                <div class="row">
                                                    ...
                                                    <div class="col-3">
                                                        ...
                                                        <select class="form-control select2" name="funcionario" id="funcionario" required="">
                                                            <option value="-1" <%=funcionario==-1?"selected":""%>>Todos</option>
                                                            <%for (usuario f : listadoFuncionarios) {%>
                                                            <option value="<%= f.getId_usuario()%>" <%=funcionario==f.getId_usuario()?"selected":""%>><%= f.getApellido()%> <%= f.getNombre()%></option>
                                                            <%}%>
                                                        </select>
                                                    </div>
                                                    <div class="col-3">
                                    ...
                                    <div class="card">
                                        <div class="card-body">
                                            <ul class="nav nav-tabs" id="myTab" role="tablist">
                                                <li class="nav-item">
                                                    <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true"><i class="fas fa-users"></i> Registros <%=fecha_inicio == null && fecha_fin == null ? "recientes" : "consultados"%> <%if (fichas.size() != 0) {%><span class="badge badge-primary"><%= fichas.size()%></span><%}%></a>
                                                </li>
                                            </ul>
                                                ...
                                                <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                                                    <div class="table-responsive">
                                                        <table class="table table-striped" id="table-x">
                                                            ...
                                                            <tbody>
                                                                <%for (FichaRiesgos fic : fichas) {
                                                                %>
                                                                <tr>
                                                                    <td><a href="javascript:" data-toggle="modal" data-target="#modalDetalleVersion" data-observacion="<%= "N/A" %>" data-app="<%= "N/A" %>" data-cod="<%= fic.getCodigo() %>" data-tipo="<%= "N/A" %>" data-autor="<%= "N/A" %>" data-fecha="<%= "N/A" %>"><%= fic.getCodigo()%></a></td>
                                                                    <td><%= fic.getFecha()%></td>
                                                                    <td><%= fic.getTecnico()%></td>
                                                                    <td><%= fic.getParroquia()%></td>
                                                                    <td><%= fic.getBarrio()%></td>
                                                                    <td><%= fic.getSector()%></td>
                                                                    <td>
                                                                        <a class="btn btn-primary btn-sm active" href="javascript:" data-toggle="modal" data-target="#modalDetalleVersion" data-observacion="<%= "N/A" %>" data-app="<%= "N/A" %>" data-cod="<%= fic.getCodigo() %>" data-tipo="<%= "N/A" %>" data-autor="<%= "N/A" %>" data-fecha="<%= "N/A" %>"><i class="fa fa-eye" data-toggle="tooltip" data-original-title="Ver detalles"></i></a>
                                                                        <a target="_blank" href="descargar_archivo.control?accion=descargar_version&id_ver=<%=fic.getCodigo()%>" class="btn btn-primary btn-sm active"><i class="fa fa-file-download" data-toggle="tooltip" data-original-title="Descargar"></i></a>
                                                                    </td>
                                                                </tr>
                                                                <%}%>
                                                            </tbody>
