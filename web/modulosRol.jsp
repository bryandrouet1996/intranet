...
<%
    ...
    int id_rol = 0;
    Rol rol = null;
    ArrayList<modulo> listadoModulosCompleto = new ArrayList<>();
    try {
        ...
        rol = enlace.getRol(id_rol);
        listadoModulosCompleto = enlace.getAllModulos();
    } catch (Exception e) {
...
<html lang="es">
    <head>
        ...
        <title>Intranet Alcaldía - Módulos por rol</title>
        <!-- General CSS Files -->
    ...
    <body>
                                ...
                                <div class="col-12">
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <div class="form-row">
                                                <div class="form-group col-md-2">
                                                    <label>Id</label>
                                                    <input disabled="" type="text" class="form-control" placeholder="Nombre funcionario" name="txtid" id="txtid" value="<%= id_rol%>" required>
                                                </div>
                                                <div class="form-group col-md-4">
                                                    <label>Nombre</label>
                                                    <input disabled="" type="text" class="form-control" placeholder="Apellido funcionario" name="txtnombre" id="txtnombre" value="<%= rol.getDescripcion()%>" required>
                                                </div>
                                                <div class="form-group col-md-12">
                                                    <label>Módulos</label>
                                                    <select class="form-control select2" multiple="multiple" required name="combomods" id="combomods">
                                                        <%for (modulo m : listadoModulosCompleto) {%>
                                                        <%if (enlace.verificarRolTieneModulo(id_rol, m.getId_modulo())) {%>
                                                        <option value="<%= m.getId_modulo()%>" selected=""><%= m.getDescripcion()%></option>
                                                        <%} else {%>
                                                        <option value="<%= m.getId_modulo()%>"><%= m.getDescripcion()%></option>
                                                        <%}%>
                                                        <%}%>
                                                    </select>
                                                </div>
                                        ...
                                        <div class="card-footer text-center">
                                            <button class="btn btn-primary" onclick="actualizarModulos()">Guardar cambios</button> 
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <script>
            function actualizarModulos() {
                ...
                var rol = <%= id_rol%>;
                var modulos = [];
                for(var opt of document.getElementById("combomods").options){
                    if(opt.selected){
                        modulos.push(opt.value);
                    }
                }
                $.post('administrar_rol_perfil.control?accion=modificar_modulos_rol', {
                    admin: admin,
                    rol: rol,
                    modulos: modulos.toString()
                }, function (responseText) {
                    if (responseText) {
                        swal("Módulos actualizados", "Se actualizaron los módulos del rol", {
                            icon: "success",
                            buttons: {
                                confirm: {
                            ...
                            location.href = "modulosRol.jsp?id_rol="+<%= id_rol%>;
                        });
                    } else {
                        swal("Error", "No se pudieron actualizar los módulos", {
                            icon: "warning",
                            buttons: {
                                confirm: {
