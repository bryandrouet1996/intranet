<%-- 
    ...
    Created on : 06/10/2021, 14:25:32
            ...
            <div class="main-wrapper main-wrapper-1">
                ...
                <div class="main-content">
                    <%
                        ...
                        ArrayList<cargo> listadoCargos = enlace.listadoCargos();
                        ArrayList<organizacion> listadoOrganizacion = enlace.listadoUnidades();
                        ArrayList<rol_usuario> listadoRoles = enlace.listadoRolesUsuarios();
                        int tipo_usuario = enlace.tipoUsuarioID(id_usuario);
                    %>
                    <section class="section">
                        <div class="section-header">
                            <h1>Modificar información de funcionario</h1>
                                ...
                                <div class="col-12">
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <div class="form-row">
                                                <div class="form-group col-md-4">
                                                    <label>Nombre</label>
                                                    <input type="text" class="form-control" placeholder="Nombre funcionario" name="txtnombre" id="txtnombre" value="<%= elemento.getNombre()%>" required>
                                                </div>
                                                <div class="form-group col-md-4">
                                                    <label>Apellidos</label>
                                                    <input type="text" class="form-control" placeholder="Apellido funcionario" name="txtapellido" id="txtapellido" value="<%= elemento.getApellido()%>" required>
                                                </div>
                                                <div class="form-group col-md-4">
                                                    <label>Cédula</label>
                                                    <input type="text" class="form-control" placeholder="Cédula funcionario" name="txtcedula" id="txtcedula" value="<%= elemento.getCedula()%>" required>
                                                </div>
                                                <div class="form-group col-md-5">
                                                    <label>Correo electrónico</label>
                                                    <input type="text" class="form-control" placeholder="Correo funcionario" name="txtcorreo" id="txtcorreo" value="<%= elemento.getCorreo()%>" required>
                                                </div>
                                                <div class="form-group col-md-7">
                                                    <label>Cargo</label>
                                                    <select class="form-control select2" required name="combocargo" id="combocargo">
                                                        <option>Seleccione</option>
                                                        <%for (int paso = 0; paso < listadoCargos.size(); paso++) {%>
                                                        <%if (elemento.getCodigo_cargo().equalsIgnoreCase(listadoCargos.get(paso).getCodigo_cargo())) {%>
                                                        <option value="<%= listadoCargos.get(paso).getCodigo_cargo()%>" selected><%= listadoCargos.get(paso).getDescripcion()%></option>
                                                        <%} else {%>
                                                        <option value="<%= listadoCargos.get(paso).getCodigo_cargo()%>"><%= listadoCargos.get(paso).getDescripcion()%></option>
                                                        <%}%>
                                                        <%}%>
                                                    </select>
                                                </div>
                                                <div class="form-group col-md-7">
                                                    <label>Unidad</label>
                                                    <select class="form-control select2" required name="combounidad" id="combounidad">
                                                        <option>Seleccione</option>
                                                        <%for (int paso = 0; paso < listadoOrganizacion.size(); paso++) {%>
                                                        <%if (elemento.getCodigo_unidad().equalsIgnoreCase(listadoOrganizacion.get(paso).getNivel_hijo())) {%>
                                                        <option value="<%= listadoOrganizacion.get(paso).getNivel_hijo()%>" selected><%= listadoOrganizacion.get(paso).getNombre()%></option>
                                                        <%} else {%>
                                                        <option value="<%= listadoOrganizacion.get(paso).getNivel_hijo()%>"><%= listadoOrganizacion.get(paso).getNombre()%></option>
                                                        <%}%>
                                                        <%}%>
                                                    </select>
                                                </div>
                                                <div class="form-group col-md-4">
                                                    ...
                                                    <select class="form-control" required name="combotipo" id="combotipo">
                                                        <option>Seleccione</option>
                                                        <%for (int paso = 0; paso < listadoRoles.size(); paso++) {%>
                                                        <%if (tipo_usuario == listadoRoles.get(paso).getId_rol()) {%>
                                                        <option value="<%= listadoRoles.get(paso).getId_rol()%>" selected=""><%= listadoRoles.get(paso).getDescripcion()%></option>
                                                        <%} else {%>
                                                        <option value="<%= listadoRoles.get(paso).getId_rol()%>"><%= listadoRoles.get(paso).getDescripcion()%></option>
                                                        <%}%>
                                                        <%}%>
                                                    </select>
                                                </div>
                                        ...
                                        <div class="card-footer text-center">
                                            <button class="btn btn-primary" onclick="modificarUsuario(<%= elemento.getId_usuario() %>)">Guardar cambios</button> 
                                        </div>
                                    </div>
                                </div>
                            </div>
