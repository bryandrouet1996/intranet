...
<%
    ...
    ArrayList<Documento> listadoBorradores = null;
    ...
    try {
        ...
        listadoBorradores = request.getParameter("txtini") == null ? mysql.getDocumentosEnviados(informacion.getId_usuario(), 0) : mysql.getDocumentosEnviados(informacion.getId_usuario(), 0, fecha_inicio, fecha_fin);
    } catch (Exception e) {
...
<html lang="es">
    <head>
        ...
        <title>Intranet Alcaldía - Borradores</title>
        <!-- General CSS Files -->
    ...
    <body>
        <div id="app">
            <div class="main-wrapper main-wrapper-1">
                    ...
                    <aside id="sidebar-wrapper">
                        ...
                        <ul class="sidebar-menu">
                            <li class="menu-header">COMPONENTES</li>
                                ...
                                %>
                            <li class="<%= c.getRuta_enlace().equals("documentos_borradores") ? "active" : ""%>"><a class="nav-link" href="sesion.control?accion=<%= c.getRuta_enlace()%>&iu=<%= id%>"><i class="<%= c.getIcono()%>"></i> <span><%= c.getDescripcion()%></span></a></li>
                                <%}%>
                        </ul>
                    </aside>
                </div>
                                    ...
                                    <div class="card">
                                        <div class="card-body">
                                            <ul class="nav nav-tabs" id="myTab" role="tablist">
                                                <li class="nav-item">
                                                    <a class="nav-link active" id="profile-tab" data-toggle="tab" href="#profile" role="tab" aria-controls="profile" aria-selected="true"><i class="fas fa-file-contract"></i> Borradores <%if (listadoBorradores.size() != 0) {%><span class="badge badge-primary"><%= listadoBorradores.size()%></span><%}%></a>
                                                </li>
                                            </ul>
                                                ...
                                                <div class="tab-pane fade show active" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                                                    <div class="table-responsive">
                                                        <table class="table table-striped" id="table-x">
                                                            ...
                                                            <tbody>
                                                                <%for (Documento d : listadoBorradores) {%>
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <a href="administrar_documento.control?accion=reporte_borrador&id=<%= d.getId()%>" target="_blank" class="btn btn-primary btn-sm" data-toggle="tooltip" data-original-title="Ver documento"><i class="fas fa-print"></i></a>
                                                                        ...
                                                                        <a onclick="abrir_modal_anexos(<%= d.getId()%>)" type="button" data-toggle="modal" class="btn btn-primary btn-sm active"><i class="fa fa-folder-open" data-toggle="tooltip" data-original-title="Ver anexos"></i></a>
                                                                            <%}%>
                                                                        <a href="ges_doc_nuevo_documento.jsp?id_documento=<%= d.getId()%>" target="_self" class="btn btn-primary btn-sm" data-toggle="tooltip" data-original-title="Editar documento"><i class="fas fa-pen"></i></a>
                                                                        ...
                                                                        <a onclick="eliminar_borrador(<%= d.getId()%>)" class="btn btn-primary btn-sm active"><i class="fa fa-times" data-toggle="tooltip" data-original-title="Eliminar"></i></a>
                                                                    </td>
                                                                </tr>
                                                                <%}%>
                                                            </tbody>
                ...
                <div class="modal fade" role="dialog" aria-hidden="true" id="modal_reasignar">
                    <div class="modal-dialog modal-lg" role="dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title text-center">
        ...
        <script>
            ...
            function abrir_modal_reasignar(id_documento) {
                ...
                $("#body_modal_reasignar").load("ges_doc_listado_reasignar_borrador.jsp?id_documento=" + id_documento, function (response, status, xhr) {
                    if (status === "success") {
                        Swal.close()
            ...
            function eliminar_borrador(id_documento){
                Swal.fire({
                    title: '¿Deseas eliminar el borrador?',
                    icon: 'warning',
                    ...
                    if (action.isConfirmed) {
                        ...
                        $.ajax({
                            url: 'administrar_documento.control?accion=eliminar_borrador',
                            type: 'POST',
                            data: formData,
                            contentType: false,
                            cache: false,
                            ...
                            error: function () {
                                Swal.fire({
                                    title: 'Error crítico',
                                    text: "No se eliminó el borrador",
                                    icon: "error",
                                    buttonsStyling: false,
                                    customClass: {
                                        confirmButton: 'btn btn-success'
