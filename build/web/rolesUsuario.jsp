...
<%
    ...
    ArrayList<rol_usuario> listadoRoles = new ArrayList<>();
    int tipo_usuario = 0;
    try {
        ...
        elemento = enlace.buscar_usuarioID(id_usuario);
        listadoRoles = enlace.listadoRolesUsuarios();
        tipo_usuario = enlace.tipoUsuarioID(id_usuario);
    } catch (Exception e) {
...
<html lang="es">
    <head>
        ...
        <title>Intranet Alcaldía - Roles de usuario</title>
        <!-- General CSS Files -->
    ...
    <body>
                                ...
                                <div class="col-12">
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <div class="form-row">
                                                <div class="form-group col-md-4">
                                                    <label>Nombre</label>
                                                    <input type="text" readonly="" class="form-control" placeholder="Nombre funcionario" name="txtnombre" id="txtnombre" value="<%= elemento.getNombre()%>" required>
                                                </div>
                                                <div class="form-group col-md-4">
                                                    <label>Apellidos</label>
                                                    <input type="text" readonly="" class="form-control" placeholder="Apellido funcionario" name="txtapellido" id="txtapellido" value="<%= elemento.getApellido()%>" required>
                                                </div>
                                                <div class="form-group col-md-4">
                                                    <label>Cédula</label>
                                                    <input type="text" readonly="" class="form-control" placeholder="Cédula funcionario" name="txtcedula" id="txtcedula" value="<%= elemento.getCedula()%>" required>
                                                </div>
                                                <div class="form-group col-md-12">
                                                    <label>Roles</label>
                                                    <select class="form-control select2" multiple="multiple" required name="combotipo" id="combotipo">
                                                        <%for (int paso = 0; paso < listadoRoles.size(); paso++) {%>
                                                        <%if (enlace.verificarUsuarioCumpleRol(id_usuario, listadoRoles.get(paso).getId_rol())) {%>
                                        ...
                                        <div class="card-footer text-center">
                                            <button class="btn btn-primary" onclick="actualizarRoles()">Guardar cambios</button> 
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <script>
            function actualizarRoles() {
                var admin = <%= informacion.getId_usuario()%>;
                var usu = <%= id_usuario%>;
                var roles = [];
                for(var opt of document.getElementById("combotipo").options){
                    if(opt.selected){
                        roles.push(opt.value);
                    }
                }
                $.post('administrar_rol_perfil.control?accion=modificar_rol', {
                    admin: admin,
                    usu: usu,
                    roles: roles.toString()
                }, function (responseText) {
                    if (responseText) {
                        swal("Roles actualizados", "Se actualizaron los roles del usuario", {
                            icon: "success",
                            buttons: {
                                confirm: {
                            ...
                            location.href = "rolesUsuario.jsp?iu="+<%= id_usuario%>;
                        });
                    } else {
                        swal("Error", "No se pudieron actualizar los roles", {
                            icon: "warning",
                            buttons: {
                                confirm: {
