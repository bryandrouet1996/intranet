 ...
 * @author Kevin Druet
 */
@WebServlet(name = "reporte_informe", urlPatterns = {"/reporte_informe.control"})
public class reporte_informe extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (tipo.equalsIgnoreCase("licencia")) {
            id_informe=Integer.parseInt(request.getParameter("ii"));
            ServletOutputStream out = response.getOutputStream();
            try {
                String jrxmlFileName = "/WEB-INF/classes/reporte/informe_licencia.jrxml";
                ...
                parametro.put("id_informe", id_informe);
        ...
        }else if (tipo.equalsIgnoreCase("lotaip")) {
            String mes=request.getParameter("combomes");
            String anio=request.getParameter("txtanio");
            ServletOutputStream out = response.getOutputStream();
            try {
                String jrxmlFileName = "/WEB-INF/classes/reporte/lotaip_n_mes.jrxml";
                File archivoReporte = new File(request.getRealPath(jrxmlFileName));
                if (archivoReporte.getPath() == null) {
                    System.out.println("No encuentro el archivo del reporte.");
                    System.exit(2);
